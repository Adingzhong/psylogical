# -*- coding: utf-8 -*-
"""
gen_B3_layouts.py (anti-regularity enhanced)
B3：黑白水果交替（N=6 => M=11）

输出：
  layouts/B3/N6_v1_layout1~3.json
  layouts/B3/_previews/N6_v1_layout*_stim.png
  layouts/B3/_previews/N6_v1_layout*_dev.png

核心增强：
- 反规律约束（允许降档）：long_ratio、quad_switches、near_neighbor_ratio、rank_correlation
- 仍保留：最小中心距、步长范围、覆盖率、不交叉、线不穿点
- 仍保留：生成失败兜底（不会崩）
"""
...
class Profile:
    ...
    # 反规律（越严格越“去规律”）
    long_step_frac: float          # 长跳阈值（基于 min(canvas_w,canvas_h)）
    min_long_ratio: float          # 长跳比例下限
    min_quad_switches: int         # 连续步跨象限次数下限
    max_abs_rank_corr: float       # step序号 与 x/y 排名相关性上限
    max_near_neighbor_ratio: float # 下一点处于“k近邻”的比例上限（越小越不局部）
...
def near_neighbor_ratio(pts: List[Pt], k: int = 3) -> float:
    # 计算每一步：下一节点在“当前节点到其余节点距离排序”里是否属于前k近邻
    if len(pts) < 2:
        return 0.0
    hits = 0
...
def score_layout(metrics: Dict[str, float]) -> float:
    # 目标：覆盖大 + 长跳足够 + 象限切换多 + 去相关 + 少近邻
    return (
...
# =========================
# Engine A：网格抽cell（允许重复cell，靠 min_center 防重叠）
# =========================
GRID_COLS = 4
GRID_ROWS = 3
...
def engineA_generate(rng: random.Random, prof: Profile) -> Optional[Tuple[List[Pt], Dict[str, float]]]:
    min_center = NODE_DIAM_PX * prof.min_center_mult
    ...
    if best_pts is None:
        return None
    return best_pts, best_metrics  # type: ignore
...
def reorder_try_many(rng: random.Random, point_set: List[Pt], prof: Profile) -> Optional[Tuple[List[Pt], Dict[str, float]]]:
    best_pts = None
    best_sc = -1e18
    best_metrics = None
    ...
    for _ in range(520):
        ...
        # 早停：已经很去规律
        if best_metrics is not None and best_metrics["near_neighbor_ratio"] < 0.48 and best_metrics["long_ratio"] > 0.50:
            break
...
def engineB_generate(rng: random.Random, prof: Profile) -> Optional[Tuple[List[Pt], Dict[str, float]]]:
    min_center = NODE_DIAM_PX * prof.min_center_mult
    ...
    for _ in range(ENGINE_B_TRIES):
        ...
        if ps is None:
            continue
        ret = reorder_try_many(rng, ps, prof)
        if ret is None:
            continue
        pts, metrics = ret
        sc = score_layout(metrics)
        ...
        if best_metrics is not None and min(best_metrics["cover_w"], best_metrics["cover_h"]) > 0.78:
            break
...
# =========================
# 总生成：seed jitter + profile downgrade + engine fallback
# =========================
def generate_best_layout(seed0: int) -> Tuple[List[Pt], Dict[str, float], bool]:
    ...
    for jitter in range(SEED_JITTER_MAX + 1):
        ...
        for pi, prof in enumerate(PROFILES):
            ...
            ret = engineA_generate(rng, prof)
            used_engine = "A"

            # Engine B
            if ret is None:
                rng = random.Random(seed * 131 + 17)
                ret = engineB_generate(rng, prof)
                used_engine = "B"
            ...
            out_metrics.update({
                ...
                "min_center_mult": float(prof.min_center_mult),
                "min_step_frac": float(prof.min_step_frac),
                "avoid_line_mult": float(prof.avoid_line_mult),
                "score": float(sc),
            })

            # strict + jitter=0 优先返回
            ...
            # 早停：mid以上且 near_neighbor 已足够低
            if prof.name in ("strict", "mid") and out_metrics["near_neighbor_ratio"] < 0.58 and min(out_metrics["cover_w"], out_metrics["cover_h"]) > 0.74:
                return pts, out_metrics, degraded
    ...
    pts = [random_point(rng) for _ in range(M)]
...
def main() -> None:
    ...
    for lid in (1, 2, 3):
        ...
        pts, metrics, degraded = generate_best_layout(seed0)
        ...
        title = (
            f"B3 {VERSION} layout{lid} start={fruit_a} node={NODE_DIAM_PX}px "
