...
MAX_SECONDS_PER_LAYOUT = 45.0
# =========================
# ✅ L（Low interference）额外约束
# - “下一正确节点”通常应比“其他错误节点”更近，减少干扰
#   对每一步 i：ratio_i = min_wrong_dist(i) / correct_dist(i)
#   要求 min_ratio 和 mean_ratio 达标
# =========================
MIN_WRONG_RATIO = 1.12   # 每一步最小阈值（越大越“简单”）
MIN_MEAN_RATIO  = 1.30   # 全程平均阈值
MAX_STEP_FRAC_CAP = 0.82 # 限制单步最长距离占对角线比例（防止跨屏超长线段）
    # 单个 layout 总上限（防卡死）
...
def try_build_with_beam(rng: random.Random, m: int, c: Constraints, b3_pts: Optional[List[Pt]]) -> Optional[Tuple[List[Pt], Dict[str, float]]]:
    ...
    for st in beam:
        ...
        # ✅ L：干扰控制（下一正确节点应该相对更近）
        # ratio_i = min_wrong_dist(i) / correct_dist(i)
        ratios = []
        ok_ratio = True
        for i in range(m - 1):
            d_corr = steps[i]
            # 限制单步最长距离，避免“跨屏大跳”
            if d_corr > MAX_STEP_FRAC_CAP * diag:
                ok_ratio = False
                break
            d_min_wrong = 1e18
            pi = pts[i]
            ...
            if d_corr < 1e-6:
                ok_ratio = False
                break
            ratios.append(d_min_wrong / d_corr)
        if (not ok_ratio) or (min(ratios) < MIN_WRONG_RATIO) or ((sum(ratios) / len(ratios)) < MIN_MEAN_RATIO):
            continue
        ...
        metrics = {
            ...
            "min_wrong_ratio": float(min(ratios)),
            "mean_wrong_ratio": float(sum(ratios) / max(1, len(ratios))),
